function [] = SimAndPlot_SIS_BinaryVsODE_RInf(Parameters)
...
%   Simulates the SIS model using both binary and ODE model
   ...
   %% Setup
    ...
    parfor i = 1:length(Parameters.SteadyState.betaValues)
        ...
        % simulate ODE model
        probabilities = SimulateNetwork_SIS_ODE(initialNodes, ...
            adjacencyMatrix, beta, gamma, ...
            simLength, deltaT);
    ...
    if Parameters.saveFig
        ...
        figFileName = ['Figures/BinaryVsODE_RInf_', datestr(now,dateTimeFormat), '.fig'];
        savefig(figFileName);
    end
